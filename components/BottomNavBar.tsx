...
export const BottomNavBar: React.FC<BottomNavBarProps> = ({ activeTab, setActiveTab }) => {
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-lg z-50">
            <div className="flex justify-around items-center h-20 shadow-[0_-2px_10px_rgba(0,0,0,0.05)] rounded-t-3xl mx-2">
                 {navItems.map((item) => (
                    <button
                        key={item.id}
                    ...
                    >
                        <div className={`w-8 h-8 transition-colors duration-300 ${activeTab === item.id ? 'text-primary' : 'text-neutral-400'}`}>
                            {item.icon}
                        </div>
                        {activeTab === item.id && (
                            <div className="absolute -bottom-1 w-8 h-1 bg-primary rounded-full"></div>
                        )}
                    </button>
                ))}
            </div>
