...
const SettingItem: React.FC<SettingItemProps> = ({ icon, title, subtitle, rightElement, onClick, showArrow = true }) => (
    <div
        onClick={onClick}
        className={`flex items-center justify-between p-4 bg-white dark:bg-neutral-800 rounded-2xl mb-3 transition-colors ${onClick ? 'cursor-pointer hover:bg-gray-50 dark:hover:bg-neutral-700 transition-colors' : ''}`}
    >
...
const ToggleSwitch: React.FC<ToggleSwitchProps> = ({ enabled, onChange }) => (
    <button
        ...
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
            enabled ? 'bg-blue-500 dark:bg-blue-600' : 'bg-gray-300 dark:bg-neutral-600'
        }`}
    >
        <span
...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ navigate, setView, onBack }) => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-neutral-900 pb-24 transition-colors">
            <div className="bg-white dark:bg-neutral-800 pt-12 pb-6 px-6 transition-colors">
                <div className="flex items-center mb-6">
                    {onBack && (
                        <button onClick={onBack} className="mr-4 text-gray-700 dark:text-neutral-300">
                            <BackArrowIcon />
                        </button>
                    )}
            ...
            <div className="px-6 py-4">
                ...
                <div className="mb-6">
                    ...
                    <SettingItem
                        ...
                        rightElement={<ToggleSwitch enabled={darkMode} onChange={(enabled) => {
                            if (enabled !== darkMode) {
                                toggleTheme();
                            }
                        }} />}
                ...
                {/* Data Management */}
                <div className="mb-6">
                    <h2 className="text-lg font-bold text-gray-900 dark:text-neutral-100 mb-3">Data</h2>
                    <SettingItem
                        icon={<SettingsIcon />}
                        title="Export Data"
                ...
                <div className="mt-8">
                    <button
                        ...
                        className="w-full bg-red-500 dark:bg-red-600 text-white py-4 rounded-2xl font-semibold hover:bg-red-600 dark:hover:bg-red-700 transition-colors"
                    >
                        Logout
                    </button>
                </div>
